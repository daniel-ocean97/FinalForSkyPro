    ...
    if request.method == 'POST':
        if step == '1':
            ...
            try:
                ...
                tables_with_availability = get_available_tables(selected_date, selected_time, guests_count)
                
                # Создаем форму с доступными столиками
                available_table_ids = [t.id for t in tables_with_availability if t.available]
                form = ReservationStep2Form(request.POST)
                form.fields['table'].queryset = Table.objects.filter(id__in=available_table_ids)
